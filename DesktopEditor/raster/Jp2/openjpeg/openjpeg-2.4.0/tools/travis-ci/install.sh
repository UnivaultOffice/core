...
if [ "${OPJ_CI_SKIP_TESTS:-}" != "1" ]; then
	...
	if [ "${OPJ_NONCOMMERCIAL:-}" == "1" ]; then
		if [ "${TRAVIS_OS_NAME:-}" == "linux" ] || uname -s | grep -i Linux &> /dev/null; then
			...
			wget -q http://kakadusoftware.com/wp-content/uploads/2026/06/KDU77_Demo_Apps_for_Linux-x86-64_150710.zip
			...
			echo "Retrieving Kakadu"
			wget -v http://kakadusoftware.com/wp-content/uploads/2026/06/KDU77_Demo_Apps_for_OSX109_150710.dmg_.zip
			...
			echo "Retrieving Kakadu"
			wget -q http://kakadusoftware.com/wp-content/uploads/2026/06/KDU77_Demo_Apps_for_Win32_150710.msi_.zip
