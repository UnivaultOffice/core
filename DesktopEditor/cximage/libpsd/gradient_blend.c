	...
	psd_int center_x, psd_int center_y, psd_int angle)
{
	...
	if(color_table == NULL)
		return psd_status_malloc_failed;
	psd_gradient_color_get_table(gradient_color, color_table, 360, (unsigned char)(psd_true - reverse));
	...
	psd_int start_x, psd_int start_y, psd_int end_x, psd_int end_y)
{
	...
	if(start_x == end_x)
	{
		...
		psd_gradient_color_get_table(gradient_color, color_table, table_count / 2, (unsigned char)(psd_true - reverse));
